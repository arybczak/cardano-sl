...
runRawBasedMode
    ...
    -> Production a
runRawBasedMode unwrap wrap transport np@NodeParams {..} sscnp mkScenario = do
    let errMsg = "runRawBasedMode: someone evaluated workers number"
    let run num outSpecs =
            runRawRealMode
                num
                ...
                listeners
                outSpecs
    (ActionSpec action, outSpecsActual) <-
        run (error errMsg) mempty $ ActionSpec $ \_ _ -> unwrap mkScenario
    allWorkersNum <-
        run (error errMsg) mempty $ ActionSpec $ \_ _ -> unwrap allWorkersCount
    run allWorkersNum outSpecsActual $
...
runProductionMode
    ...
    -> NodeParams
    -> SscParams ssc
    -> WrappedSpec (ProductionMode ssc) a
    -> Production a
